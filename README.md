...
当前版本已经实现生成器与识别器的第一版能力：支持 QR Code、Code 128、EAN-13，能够实时预览、保存图片、从文件识别和通过摄像头实时识别。
...
- `services`：封装生成与识别流程。当前使用 ZXing 生成和识别 QR Code、Code 128、EAN-13。
...
说明：Qt Widgets 与 ZXing 是当前必需依赖；Qt6 Multimedia 可用时会启用摄像头识别；OpenCV 仍作为后续图像预处理能力的可选依赖。
